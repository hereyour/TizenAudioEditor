...
WaveSurfer.Regions = {
    ...
    enableDragSelection: function (params) {
        ...
        this.wrapper.addEventListener('mousedown', function (e) {
			...
			document.getElementById("desc_1_audio").value = (start * my.wavesurfer.getDuration()).toFixed(2)+"sec";
            document.getElementById("desc_2_audio").value = '';
            my.wavesurfer.clearRegions();
            region = null;
        });
        ...
        this.wrapper.addEventListener('mousemove', function (e) {
            ...
            document.getElementById("desc_1_audio").value = (Math.min(end * duration, start * duration)).toFixed(2)+"sec";
            document.getElementById("desc_2_audio").value = (Math.max(end * duration, start * duration)).toFixed(2)+"sec";
...
WaveSurfer.Region = {
    ...
    onDrag: function (delta) {
        document.getElementById("desc_1_audio").value = (this.start + delta).toFixed(2)+"sec";
        document.getElementById("desc_2_audio").value = (this.end + delta).toFixed(2)+"sec";
    ...
    onResize: function (delta, direction) {
        if (direction == 'start') {
            document.getElementById("desc_1_audio").value =  (Math.min(this.start + delta, this.end)).toFixed(2)+"sec";
            document.getElementById("desc_2_audio").value = (Math.max(this.start + delta, this.end)).toFixed(2)+"sec";
            this.update({
                start: Math.min(this.start + delta, this.end),
                end: Math.max(this.start + delta, this.end)
            });
        } else {
            document.getElementById("desc_1_audio").value = (Math.min(this.end + delta, this.start)).toFixed(2)+"sec";
            document.getElementById("desc_2_audio").value = (Math.max(this.end + delta, this.start)).toFixed(2)+"sec";
            this.update({
